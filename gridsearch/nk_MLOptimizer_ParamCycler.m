...
if ~exist('PsSel','var') || isempty(PsSel)
    ...
    for curclass=1:nclass
        PiSel(:,curclass) = PsSel{curclass}{MULTILABEL.curdim}.SelNodes;
    end 
end
...
for i = 1:nPs
    ...
    if ~sum(any(PiSel(i,:))),
        fprintf('\nSkipping parameter combination %g!',i); continue; 
    end
    ii = [ii i];
    ...
    if nPs > 1 
        DISP.s = sprintf('%s | %s: CV2 [ %g, %g ] => %4g/%4g parameter combinations => %1.1f%% ', ...
            elaps, algostr, f, d , pltcnt, pltmax, pltperc);
    else
        DISP.s = sprintf('%s | %s: CV2 [ %g, %g ] => No-parameter optimization', elaps, algostr, f, d  );
    end
    fprintf('%s',DISP.s); 
