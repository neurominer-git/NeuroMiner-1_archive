...
function NM = nk_DefineOOCVData3_config(NM, O, parentstr)
...
NM.(fldnam){inp.oocvind}.desc = Y.desc;
NM.(fldnam){inp.oocvind}.date = Y.date;
if ~isfield(NM.(fldnam){inp.oocvind},'n_subjects_all'), NM.(fldnam){inp.oocvind}.n_subjects_all = Inf; end
if ~isfield(NM.(fldnam){inp.oocvind},'labels_known') && isfield(Y,'labels_known')
    NM.(fldnam){oocvind}.labels_known = Y.labels_known; 
else
    Y.labels_known = false;
    NM.(fldnam){inp.oocvind}.labels_known = false;
end

menustr = cell(nM+1,1);
menuact = 1:nM;
if isfield(NM,'covars'), inp.covflag = true; else, inp.covflag = false; end
if oocvind, 
    OOCVSelStr = sprintf('New data #%g: %s', inp.oocvind, Y.desc); 
else, 
    OOCVSelStr = na_str; 
end;
menuact{1} = sprintf('Define/Select OOCV workspace [ %s ]', OOCVSelStr);

for i=2:nM+1
    if i>numel(NM.(fldnam){oocvind}.Y) || isempty(NM.(fldnam){inp.oocvind}.Y{i})
        menustr{i} = sprintf('OOCV Input %s for Modality %g [ NOT provided ]', dattype, i); 
    else
        menustr{i} = sprintf('OOCV Input %s for Modality %g [ provided ]', dattype, i); 
    end
end
    ...
    if isfield(NM.(fldnam){oocvind},'cases')
