...
if isfield(PREPROC,'ACTPARAM') && ~isempty(PREPROC.ACTPARAM)
    ...
    for i=1:numel(d.PREPROC.preprocact)
        ...
        if strcmp(PREPROC.ACTPARAM{i}.cmd,'extfeat')
			if i==stepind+1
				fprintf('\n\t'); cprintf('*SystemCommands','\\__ %s',stepstr);
			else
				cprintf('SystemCommands','\n\t    %s',stepstr);
            end
        elseif strcmp(PREPROC.ACTPARAM{i}.cmd,'extdim')
            if i==stepind
				fprintf('\n\t'); cprintf('*black','\\__ %s',stepstr);
			else
				fprintf('\n\t    %s',stepstr);
            end
        elseif strcmp(PREPROC.ACTPARAM{i}.cmd,'reducedim')
...
%             stepind = stepind + 2;
    ...
    case 9 % Change order of preprocessing steps
        ...
        for i = 1:numel(neworder)
            ...
            elseif strcmp(PREPROC.ACTPARAM{neworder(i)}.cmd,'reducedim') 
                if i == numel(neworder) || (i < numel(neworder) && ~strcmp(PREPROC.ACTPARAM{neworder(i+1)}.cmd,'extdim')), fl = false;  end
            end
        end
        if fl, 
...
%TrainParam.PREPROC{varind} = PREPROC;
