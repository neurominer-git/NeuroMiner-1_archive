...
if ~defaultsfl
    ...
    if W_ACT.threshvec(1)>0 || numel(W_ACT.threshvec)>1 
        threshstr = sprintf('Thresholding (Hard feature selection: %s)', nk_ConcatParamstr(W_ACT.threshvec));
        menustr = sprintf('Thresholding or weighting of features [ %s ]', threshstr); menuact = 1;
        if datadesc.type
    ...
    switch act
        ...
        case 1
            W_ACT.threshvec = nk_input('Define vector of percentile thresholds (''0'' switches to soft feature selection)',0,'e',W_ACT.threshvec);
