function x=NNQPIP(AtA,Atb,btb,AtAInv,option)
% Interior-point method to solve the single kernel NNLS/NNQP problem:
...
optionDefault.tIni=10; % the initial value of t
...
while dualGap>=option.tol && i+1<=option.NewtonMaxIter
    i=i+1;
%     fprintf('The %d-th iteration of Newtons method for NNLS ...\n',i);
...
%    [deltax,flag,relres,iter,resvec]=pcg(H,-g,pcgtol,option.maxit,M);
   % search the step size of Newton's method
    % update x
   s=1;
   oldf=0.5*t*x'*AtA*x - t*Atb'*x + 0.5*t*btb -sum(log(x));
   for j=1:option.lineSearchMaxIter
       newx=x+s*deltax;
       if min(newx>0)
           newf1=0.5*t*newx'*AtA*newx - t*Atb'*newx + 0.5*t*btb -sum(log(newx));
...
%            fprintf('infeasible solution\n');
   ...
   xtAtAx=x'*AtA*x;
   dualGap=xtAtAx - Atb'*x;
%    dualVal=max(-0.5*xtAtAx+0.5*btb,dualVal)
%    primalVal=0.5*xtAtAx - Atb'*x + 0.5*btb
...
% fprintf('NNLS via Newtons method finished.\n');
end
