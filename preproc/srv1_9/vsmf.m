function [A,Y,AtA,numIter,tElapsed,finalResidual]=vsmf(X,k,option)
...
%         curRes=norm(X-XfitThis,'fro');
         ...
         if option.kernelizeAY==0
            curRes=0.5*trace(X'*X-Y'*AtX-AtX'*Y+Y'*AtA*Y) + 0.5*option.alpha2*trace(AtA) + option.alpha1*sum(sum(abs(A))) + 0.5*option.lambda2*trace(YYt) + option.lambda1*sum(sum(abs(Y)));
        end
        
        if option.kernelizeAY==1
            curRes=0.5*trace(XtX-Y'*AtX-AtX'*Y+Y'*AtA*Y) + 0.5*option.alpha2*trace(AtA) + 0.5*option.lambda2*trace(YYt) + option.lambda1*sum(sum(abs(Y)));
        end
       
        if option.kernelizeAY==2
            curRes=0.5*trace(XXt-YXt'*A'-A*YXt+A*YYt*A') + 0.5*option.alpha2*trace(AtA) + option.alpha1*sum(sum(abs(A))) + 0.5*option.lambda2*trace(YYt);
        end
