function model=khdlmTrain(trainSet,trainClass,option)
...
model.trainClassMtKInv=trainClassM'*invsvd(K);
model.trainSetM=trainSetM;
model.trainClassM=trainClassM;
model.numCl=numUnik;
model.unikClass=unikClass;
model.option=option;
if strcmp(option.kernel,'linear')
    model.beta=pinv(trainSetM')*trainClassM(:,1);
end
end
