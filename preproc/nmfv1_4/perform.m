...
perf(1,c+2)=nm_nanmean(perf(1:c)); % geometricMean(perf(1:c)) % nm_nanmean(perf(1:c)); % bacc/arithmetric mean
end
...
% perf(1,6)=sqrt(perf(1,3)*perf(1,4)); % Geometric mean %nm_nanmean([perf(1,3);perf(1,4)]); % BACC
% end
%
...
%     end
%     perf(1,c+1)=perf(1,c+1)/n; %acc
%     perf(1,c+2)=geometricMean(perf(1:c)); % geometric mean % nm_nanmean(perf(1:c)); % bacc
% end
